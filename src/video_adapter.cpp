        ...
        if(!markerIds.empty()) {
            ...
            if (dest_point_ != cv::Point2f(0,0) && flag_laser_track_){
                ...
                if(flag_auto_trace_) {
                    ...
                    autoMotionPlanning(in_thresh_,dest_point_,laser_point_);
                }
            }
        }
    }
...
void VideoAdapter::autoMotionPlanning(bool &_stop, cv::Point2f &_dest, cv::Point2f &_current) {
    if (_stop){
    ...
    if (!_stop){
        ...
        if(abs(diffVector.x) < pixel_threshold) {
            x_stat = 0;
        }
        else {
            if(diffVector.x < 0){
        ...
        if(abs(diffVector.y) < pixel_threshold) {
            y_stat = 0;
        }
        else {
            if(diffVector.y < 0){
